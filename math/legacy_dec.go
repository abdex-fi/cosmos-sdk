...
// abdex: Dec is an alias for LegacyDec
type Dec = LegacyDec
...
func init() {
	...
	// abdex: 2^256 * 10^18 -1 -> 2^512 * 10^18 -1
	tmp := new(big.Int).Exp(big.NewInt(2), big.NewInt(512), nil)
...
// abdex: Marshal implements the gogo proto custom type interface.
...
// abdex: MarshalTo implements the gogo proto custom type interface.
...
// abdex: Unmarshal implements the gogo proto custom type interface.
